...
- **The site half is checked too, from `variant-check` 0.1.35.** `siteName`, `language`, `siteType`, `theme` (including `layout`, `spacing` and the font list), `siteSections` and `pages` are validated locally against the upload's own schema, and every wrong value is reported in ONE run (`manifest-site`). A closed enum such as `siteType` or `theme.layout` accepts only the listed values. `"company"` or `"standard"` is an error, not a synonym.
- **Check the whole bundle in one run with `npx @webto-id/variant-check site <folder>`** (0.1.37+): `template.json`, every `sections/*.astro`, and the CONTENT of every section against the schema its type and variant carry — a FAQ with `items: []` or a logo with `assets/logo.png` fails here, one line per problem (`pages[0].sections[6] (FAQ / default): items: minimal 1 item, ada 0`), instead of at the dry run.
...
3. On create: variants are created under the seller's account auto-submitted for review: approved right away, or **pending** until an admin decides (variant skill `wvf.md` §4 "Review rule"), the local refs are rewritten to real ids, and the template draft site appears under /templates.
