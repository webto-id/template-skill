...
2. **Chrome never becomes a WVF file** — the platform refuses site-scoped variants. Map the source's navbar/footer onto the closest **platform** variant (`navbar`, `banner`, `footer` types) and move its branding, links, and contact info into `content`. Site pages appear in the navbar automatically; `content.links` is only for EXTRA destinations.
