...
8. **Revisions**: to iterate on an ALREADY-uploaded template, use the listing editor's **"Update dari Bundle"** button (or /templates/upload?siteId=...). Matching is by each variant's bundle key (= file basename): identical sources are skipped ("Sama"), changed ones become a new version ("Versi baru" — re-enters review; buyers keep the approved version meanwhile), new keys become new variants. Partial uploads are supported:
