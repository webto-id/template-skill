...
The platform compiles every `.astro` **strict**, validates the whole manifest, creates the variants under the seller's account (auto-submitted: approved automatically, or after an automatic security check that may hand a scripted variant to an admin — see the variant skill's `wvf.md` §4 "Review rule"), and assembles a **template draft** the seller lists from the listing editor.
...
13. **Deliver** the folder, plus the "What will not be copied" list below filled in for THIS conversion (even if some rows don't apply) — the seller can paste it straight into the listing description. After upload: each variant is approved automatically or, after the automatic security check, waits for an admin; once all are approved, the seller publishes the listing from the template's listing editor.
